...
async function testProviderConnection(providerKey, conf) {
  const cfg = conf || (loadConfig().providers[providerKey] || {});
...
router.post('/chat', async (req, res) => {
  try {
    ...
    const activeProvider = requestedProvider || config.activeProvider || 'ollama';
    ...
    try {
      ...
      // 3. GOOGLE GEMINI
        ...
        const payload = {
          model: model || providerConfig.model || 'gemini-1.5-flash',
          messages: chatMessages,
          temperature
        };
        if (tools && Array.isArray(tools) && tools.length > 0) {
